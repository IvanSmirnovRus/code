...
class CalculatorBrain {
    private enum Op: CustomStringConvertible{
        ...
        case BaniryOperation(String, (Double, Double) -> Double)
        var description: String {
            get {
                ...
                case .UnaryOperation(let symbol, _):
                    return symbol
                case .BaniryOperation(let symbol, _):
                    return symbol
                }
            }
        }
    ...
    init() {
        knowsOps["×"] = Op.BaniryOperation("×", * )
        knowsOps["÷"] = Op.BaniryOperation("÷") { $1 / $0 }
        knowsOps["+"] = Op.BaniryOperation("+", + )
        knowsOps["−"] = Op.BaniryOperation("−") { $1 - $0 }
    ...
    private func evaluate(ops: [Op]) -> (result: Double?, remainingOps: [Op]) {
        if !ops.isEmpty {
            ...
            switch op {
                ...
                case .BaniryOperation(_, let operation):
